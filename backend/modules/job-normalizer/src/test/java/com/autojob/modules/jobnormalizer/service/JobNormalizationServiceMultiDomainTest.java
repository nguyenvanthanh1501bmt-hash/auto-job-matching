...
class JobNormalizationServiceMultiDomainTest {
    ...
    void shouldNormalizeJobsAcrossMultipleIndustries() {
        NormalizedJob it = normalize(rawJob(
                "it",
        ...
        assertThat(it.getSeniority())
                .isEqualTo(SeniorityLevel.SENIOR);

        assertThat(it.getLocations())
                .containsExactly(
                        "Hồ Chí Minh",
                        "Remote"
                );

        NormalizedJob accounting = normalize(rawJob(
                "accounting",
                "Kế toán tổng hợp",
                List.of(
                        "misa",
        ...
        assertThat(accounting.getSkills())
                .containsExactly(
                        "MISA",
                        "Kế toán thuế",
                        "Báo cáo tài chính"
        ...
        assertThat(accounting.getSeniority())
                .isEqualTo(SeniorityLevel.MID);

        assertThat(accounting.getEmbeddingText())
                .contains(
        ...
        NormalizedJob sales = normalize(rawJob(
                "sales",
        ...
        assertThat(sales.getSeniority())
                .isEqualTo(SeniorityLevel.MANAGER);

        assertThat(sales.getSkills())
                .contains(
                        "B2B Sales",
        ...
        NormalizedJob manufacturing = normalize(rawJob(
                "manufacturing",
                "Kỹ sư CNC",
                List.of(
                        "CNC",
        ...
        assertThat(manufacturing.getSkills())
                .containsExactly(
                        "CNC",
                        "AutoCAD",
                        "Đọc bản vẽ kỹ thuật"
        ...
        assertThat(manufacturing.getLocations())
                .containsExactly("Bắc Ninh");

        NormalizedJob healthcare = normalize(rawJob(
                "healthcare",
                "Điều dưỡng viên",
                List.of(
                        "Chăm sóc bệnh nhân",
        ...
        assertThat(healthcare.getSkills())
                .containsExactly(
                        "Chăm sóc bệnh nhân",
                        "Điều dưỡng"
                );

        assertThat(healthcare.getExperienceMin())
                .isEqualTo(1.5);

        assertThat(healthcare.getJobType())
                .isEqualTo(NormalizedJobType.FULL_TIME);
    }

    @Test
    void shouldUseSkillFallbackWithoutBreakingHashDrivenIdempotencyInputs() {
        RawJob rawJob = rawJob(
                "fallback-accounting",
                "Senior Accountant",
                List.of(),
                "5 years",
        ...
        NormalizedJob normalized = normalize(rawJob);

        assertThat(normalized.getSkills())
                .contains(
                        "MISA",
                        "IFRS",
        ...
        assertThat(normalized.getRawContentHash())
                .hasSize(64);
    }

    private NormalizedJob normalize(RawJob rawJob) {
        when(
        ...
        return service
                .normalizeByRawJobId(rawJob.getId())
                .execution()
                .normalizedJob();
    }
    ...
    private RawJob rawJob(
        ...
        return RawJob.builder()
                ...
                .jobTypeText("nhân viên chính thức")
                .salaryText("20tr - 30tr")
                .postedText("2 hours ago")
                .deadlineText("in 3 days")
                .descriptionText("Mô tả công việc")
                .requirementsText("Yêu cầu công việc")
                .benefitsText("Phúc lợi")
                .detailUrl(
                        "https://jobs.example.com/" + id
                )
                .applyUrl(
                        "https://company.example.com/apply/" + id
                )
                .applyType(ApplyType.UNKNOWN)
                .build();
    }
}