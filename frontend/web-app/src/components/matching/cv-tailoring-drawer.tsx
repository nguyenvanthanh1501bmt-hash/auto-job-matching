...
import {
    CvTailoringPlan
} from "@/components/matching/cv-tailoring-plan";
...
import type {
    ...
    CvTailoringPreviewScoreCardProps,
    CvTailoringScoreRow
...
function recommendedSuggestionIds(
    suggestions:
    CvTailoringSuggestionItem[]
) {
    /*
     * Prefer POSITION/REWRITE because these may change
     * candidate semantic representation.
     *
     * SURFACE still remains selectable manually.
     */
...
function PreviewScoreCard({
    ...
    const scoreRows:
        ...
        },
        {
            ...
            value:
            snapshot.locationScore,

            known:
                snapshot.locationKnown
                === true
        },
        {
            key:
...
export function CvTailoringDrawer({
    ...
    const runAnalyze =
        useCallback(
            () => {
                ...
                initializedAnalysisRef.current =
                    null;
    ...
    /*
     * Automatically choose meaningful safe changes for the first
     * preview, instead of allowing an accidental 0-change preview.
     */
    useEffect(
        () => {
            if (
                !analysis
                || initializedAnalysisRef
                    .current
                === analysis.analysisId
            ) {
                return;
                            ...
                            <section className="rounded-[22px] border border-black/[0.055] bg-white p-5 shadow-[0_5px_22px_rgba(0,0,0,0.02)] sm:p-6">
                                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                    ...
                                    <button
                                        ...
                                        className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-full bg-[#171717] px-5 text-[10px] font-semibold text-white shadow-[0_4px_14px_rgba(0,0,0,0.08)] disabled:cursor-not-allowed disabled:opacity-40"
                                    >
                                        {previewMutation.isPending ? (
